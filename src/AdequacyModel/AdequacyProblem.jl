...
struct AdequacyResult
    ...
    storage_power_dEUEs::Vector{Float64}   # n_storages (full-year sum)
    storage_energy_dEUEs::Vector{Float64}  # n_storages (full-year sum)
    storage_power_dEUEs_ts::Matrix{Float64}   # n_timesteps × n_storages
    storage_energy_dEUEs_ts::Matrix{Float64}  # n_timesteps × n_storages
...
function solve(prob::AdequacyProblem)
    ...
    stor_power_dEUE_ts  = result.storage_power_dEUEs[:,  ip]
    stor_energy_dEUE_ts = result.storage_energy_dEUEs[:, ip]

    return AdequacyResult(
        prob.sys.timesteps, result.demand, result.lolps, result.eues,
        result.generation_dEUEs,
        stor_power_dEUE,    stor_energy_dEUE,
        stor_power_dEUE_ts, stor_energy_dEUE_ts)
